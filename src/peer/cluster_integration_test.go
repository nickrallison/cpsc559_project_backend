...
func TestSimultaneousLeaderElection(t *testing.T) {
	...
	nodeA.Start()
	nodeB := peer.NewPeerServer(peer.Follower, "9305", "localhost", "localhost:9304", "localhost:9205,localhost:9306", dbB, simulateDelay, avgDelay, stdevDelay)
	nodeB.Start()
	nodeC := peer.NewPeerServer(peer.Follower, "9306", "localhost", "localhost:9304", "localhost:9305,localhost:9306", dbC, simulateDelay, avgDelay, stdevDelay)
	nodeC.Start()
